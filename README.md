...
- `assets/video/` — `singular-reel.mp4`: the gameplay reel in the Singular gameplay band, under "You heal and reload…", and inside the Singular card on the home page (source `Publish.mp4`, 1920 × 1080 / 60 fps / 650 MB, re-encoded to 1280 × 720 / 30 fps, H.264 CRF 30, original audio kept as AAC 128 kb/s, ~24 MB). Autoplays muted and looped everywhere; the speaker button beside it (`[data-vol]`, wired in `site.js`) is the only thing that turns the sound on. The home card keeps no button: the video sits inside the link. Visitors with reduced motion keep the poster
...
- `void-singularcorp.ppsx` and `void-singularcorp.pptx` — the read-only deck (below), once as a show that opens straight into the slideshow and once as a plain .pptx for the viewers that do not handle shows (phones, mostly). Same pictures, same password, same "final" flag.

**To update the deck, run that one command and the four files are replaced**; the page reads the paths from its links, so nothing else changes. To rename them, change the `assets/pitch/…` links in `pitch.html` and run `npm run build`.
